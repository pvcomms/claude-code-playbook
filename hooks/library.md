...
Run `npx @paramxclaudedev/setup` to install the core 3 automatically.
...
Full methodology in `packages/cli` — run `claude-code-hook-eval` to reproduce.
...
Or run `npx @paramxclaudedev/setup` for the core 3 (auto-format + guard + chime).
